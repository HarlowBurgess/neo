...
| G4 | **Provisional signals; a human decides.** A *strategic-reopen candidate* is raised when any of these holds: ≥2 falsified KPIs trace to one PRD success criterion or assumption; one falsified KPI traces to a P0 success criterion; ≥2 *mis-specified* verification findings trace to one PRD segment. Candidates go to the human Product Engineer. A candidate is never an automatic reopen. G4 moves to **Partial**. |
